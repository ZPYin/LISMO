function [layerInfo, PD, PN] = VDE_cld(signal, height, SNR, minLayerDepth, minHeight, smoothWin, minSNR)
...
for iLayer = 1:nLayer
    ...
    layerIndx = (L == iLayer);
    layerSNR = sqrt(nansum(noise_level(layerIndx).^2) / sum(layerIndx));
