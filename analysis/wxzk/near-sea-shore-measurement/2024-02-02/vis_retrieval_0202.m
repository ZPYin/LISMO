...
scanTime2Plot = datenum(2024, 2, 2, 0, 9, 0);   % temporal range for data input
distOffset = -48.75;
iCh = 1;
...
mAttn = (mBsc + extRef / 50) .* exp(-2 * nancumsum((mExt + extRef) .* [range(1), diff(range)]));
...
if exist('scanData', 'var')
    ...
    for iPrf = 1:size(scanData.signal, 1)
        ...
        if strcmpi(visRetMethod, 'xian')
            scanData.ext(iPrf, :) = extRet_Xian(range, scanData.signal(iPrf, :), scanData.bg(iPrf), 'minSNR', 0.1, 'rangeFullOverlap', 200);
        elseif strcmpi(visRetMethod, 'quasi')
            ...
            scanData.ext(iPrf, :) = scanData.ext(iPrf, :) * (1064 / 532).^1.4+ mExt;
        else
...
if exist('fullData', 'var')
    ...
    for iPrf = 1:size(fullData.signal, 1)
        ...
        if strcmpi(visRetMethod, 'xian')
            fullData.ext(iPrf, :) = extRet_Xian(range, fullData.signal(iPrf, :), fullData.bg(iPrf), 'minSNR', 0.1, 'rangeFullOverlap', 200);
        elseif strcmpi(visRetMethod, 'quasi')
            [~, fullData.ext(iPrf, :)] = extRet_Holger(range, fullData.signal(iPrf, :), ...
                'calibration_constant', lcMean, ...
                'fullOverlapR', 100, ...
                'elevation_angle', scanData.zenithAng(1));
            fullData.ext(iPrf, :) = fullData.ext(iPrf, :) * (1064 / 532).^1.4 + mExt;
        else
            warning('Undefined extinction retrieval method.');
        end
