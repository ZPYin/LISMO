...
dataFolder = 'C:\Users\ZPYin\Documents\Data\wxzk_fog_measurements\海试数据、效果图\无人艇12月18、29数据\12.18';   % input a scanning period. Path of scanning data.
tRange = [datenum(2023, 12, 18, 7, 45, 0), datenum(2023, 12, 18, 11, 19, 0)];   % temporal range for data input
tCaliRange = [datenum(2023, 12, 18, 11, 0, 0), datenum(2023, 12, 18, 11, 10, 0)];
distOffset = 55.75;
olHeight = 300;   % 
refDist = [800, 1000];
calDist = [500, 800];
...
flagReadData = false;
overlapCor = true;
overlapFile = 'overlap_20231218.mat';
...
%% lidar calibration
isInCaliRange = (lData.startTime >= tCaliRange(1)) & (lData.startTime <= tCaliRange(2));
...
for iPrf = 1:length(lData.hRes)
         ...
         if (lData.startTime <= datenum(2023, 12, 18, 8, 15, 0))
...
s2 = plot(lData.startTime, vis(:, 50) * 1e-3, 'color', 'b', 'Marker', '.', 'MarkerFaceColor', 'b', 'markeredgecolor', 'b', 'DisplayName', sprintf('激光测雾雷达(%3d米)', floor(range(50))));
hold off;
