...
dataFolder = 'C:\Users\ZPYin\Documents\Data\wxzk_fog_measurements\海试数据、效果图\10月21长航实验原始数据';   % input a scanning period. Path of scanning data.
tRange = [datenum(2023, 10, 21, 6, 30, 0), datenum(2023, 10, 21, 21, 14, 0)];   % temporal range for data input
% tRange = [datenum(2023, 10, 21, 18, 30, 0), datenum(2023, 10, 21, 21, 14, 0)];   % temporal range for data input
tCaliRange = [datenum(2023, 10, 21, 19, 30, 0), datenum(2023, 10, 21, 19, 32, 0)];% fog lidar with polarization
distOffset = 55.75;
olHeight = 300;   % 
refDist = [5000, 6000];
calDist = [1500, 2000];
...
bg = nanmean(squeeze(lData.rawSignal(:, 3, (end - 15):(end - 1))), 2);
...
p1 = pcolor(lData.startTime, range * 1e-3, transpose(rcsTmp));
p1.EdgeColor = 'none';
caxis([0, 3e9]);
colormap('jet');
...
%% lidar calibration
isInTCaliRange = (lData.startTime >= tCaliRange(1)) & (lData.startTime <= tCaliRange(2));
isInHCaliRange = (range >= calDist(1)) & (range <= calDist(2));
meanRCS = nanmean(rcs(isInTCaliRange, :), 1);
...
[~, extRef] = chi2fit(range(isInHCaliRange), log(meanRCS(isInHCaliRange)), 0.1 * ones(size(meanRCS(isInHCaliRange))));
extRef = extRef / -2;
extRefStd = nanstd(extSlopeMethod(isInRefDist));

aBsc = fernald(range, nanmean(signal(isInTCaliRange, :), 1), nansum(bg(isInTCaliRange)), lr, refDist, extRef / lr, mBsc, 4);
...
lcMean = nanmean(lc(isInHCaliRange));
lcStd = nanstd(lc(isInHCaliRange));
meanL2M = nanmean(meanRCS(isInHCaliRange)) / nanmean(mAttn(isInHCaliRange));
...
rcsTmp = rcs;
rcsTmp(lowSNRMask) = NaN;
p1 = pcolor(lData.startTime, range * 1e-3, transpose(rcsTmp) / lcMean * 1e6);
p1.EdgeColor = 'none';
caxis([0, 2]);
colormap('jet');
...
for iPrf = 1:length(lData.hRes)
    ...
    if strcmpi(visRetMethod, 'xian')
        ext(iPrf, :) = extRet_Xian(range, rcs(iPrf, :) ./ range.^2, bg(iPrf), 'minSNR', 1, 'rangeFullOverlap', 600);
...
s2 = plot(lData.startTime, vis(:, 100) * 1e-3, 'color', 'b', 'Marker', '.', 'MarkerFaceColor', 'b', 'markeredgecolor', 'b', 'DisplayName', sprintf('激光测雾雷达(%3d米)', floor(range(100))));
hold off;
